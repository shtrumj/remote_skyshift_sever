...
from auth import get_current_active_user, get_current_admin_user, get_current_approved_user, User, UserCreate, UserLogin, create_access_token, get_password_hash, verify_password
from database import db_manager
from shared import agent_manager, manager
from shared import AgentRegistration, HeartbeatRequest, CommandRequest, CustomerRegistration, ScriptRegistration, ScriptExecutionRequest
import uuid
...
async def login_user(user_credentials: UserLogin):
    ...
    try:
        ...
        if not user_data["is_admin"] and not user_data["is_approved"]:
            raise HTTPException(status_code=401, detail="Account not yet approved by admin")
...
async def get_profile(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        # Add placeholder statistics for now
        user_data.update({
            "total_logins": 0,
            "session_time": "0h",
            "commands_sent": 0,
        ...
        logger.info(f"✅ Profile returned successfully for user: {current_user.username}")
        return user_data
    except HTTPException:
        raise
    except Exception as e:
...
async def change_password(
    ...
    try:
        ...
        success = db_manager.update_user(current_user.id, {"hashed_password": hashed_password})
...
async def list_online_agents():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list online agents: {str(e)}")
...
async def reject_user(user_id: str, current_user: User = Depends(get_current_admin_user)):
...
async def make_user_admin(user_id: str, current_user: User = Depends(get_current_admin_user)):
...
async def remove_user_admin(user_id: str, current_user: User = Depends(get_current_admin_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to remove admin privileges: {str(e)}") 