...
from auth import get_password_hash, UserCreate
import uuid
...
def create_user(username: str, password: str, email: str = None, full_name: str = None):
    ...
    try:
        ...
        if user_id:
            ...
            print(f"   Created: {user_data['created_at'].strftime('%Y-%m-%d %H:%M:%S')}")
            return user_data
        else:
...
Examples:
  ...
  python create_user.py jane password123 jane@example.com "Jane Smith"
    ...
    parser.add_argument(
        "--full-name", "-f",
        help="Full name for the new user (default: username)"
    )
